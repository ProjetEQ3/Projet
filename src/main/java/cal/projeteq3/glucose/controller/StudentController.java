...
public class StudentController{
	...
	public ResponseEntity<CvFileDTO> getCv(@PathVariable Long studentId){
		return ResponseEntity.accepted().contentType(MediaType.APPLICATION_JSON).body(studentService.getCv(studentId));
	}

	@PostMapping("/cv/{studentId}")
	public ResponseEntity<CvFileDTO> addCv(
		@PathVariable Long studentId, @RequestParam("file") MultipartFile file
	) throws IOException{
	...
	public ResponseEntity<List<JobOfferDTO>> getJobOffersByDepartment(
		@PathVariable String department, @RequestParam String season, @RequestParam String year
	){
		...
		return ResponseEntity.accepted().contentType(MediaType.APPLICATION_JSON).body(
			studentService.getJobOffersByDepartment(Department.valueOf(department), semester));
	}

	@GetMapping("/jobOffers/open/{department}")
	public ResponseEntity<List<JobOfferDTO>> getOpenJobOffersByDepartment(
		@PathVariable String department, @RequestParam String season, @RequestParam String year
	){
		Semester semester = new Semester(Semester.Season.valueOf(season), Integer.parseInt(year));

		return ResponseEntity.accepted().contentType(MediaType.APPLICATION_JSON).body(
			studentService.getOpenJobOffersByDepartment(Department.valueOf(department), semester));
	}

	@PostMapping("/applyJobOffer/{studentId}/{jobOfferId}")
	public ResponseEntity<JobOfferDTO> applyJobOffer(
		@PathVariable Long studentId, @PathVariable Long jobOfferId, @RequestBody String coverLetter
	){
	...
	public ResponseEntity<List<JobOfferDTO>> getAppliedJobOfferByStudentId(
		@PathVariable Long studentId, @RequestParam String season, @RequestParam String year
	){
		return ResponseEntity.accepted().contentType(MediaType.APPLICATION_JSON).body(
			studentService.getAppliedJobOfferByStudentId(
				studentId,
			  Semester.builder().season(Semester.Season.valueOf(season)).year(Integer.parseInt(year)).build())
		);
	}
	...
	public ResponseEntity<List<AppointmentDTO>> getAppointmentsByJobApplicationId(@PathVariable Long id){
		return ResponseEntity.accepted().contentType(MediaType.APPLICATION_JSON).body(
			studentService.getAppointmentsByJobApplicationId(id));
	}

	@GetMapping("/appointmentsByJobOfferIdAndStudentId/{jobOfferId}/{studentId}")
	public ResponseEntity<List<AppointmentDTO>> findAllAppointmentsForJobOfferAndStudent(
		@PathVariable Long jobOfferId, @PathVariable Long studentId
	){
		return ResponseEntity.accepted().contentType(MediaType.APPLICATION_JSON).body(
			studentService.findAllAppointmentsForJobOfferAndStudent(jobOfferId, studentId));
	}

	@PutMapping("/setAppointmentToChosen/{id}")
	public ResponseEntity<AppointmentDTO> setAppointmentToChosen(@PathVariable Long id){
		return ResponseEntity.accepted().contentType(MediaType.APPLICATION_JSON).body(
			studentService.setAppointmentToChosen(id));
	}

	@GetMapping("/contracts/{studentId}")
	public ResponseEntity<List<ContractDTO>> getContractsByStudentId(
		@PathVariable Long studentId, @RequestParam String season, @RequestParam String year
	){
		return ResponseEntity.accepted().contentType(MediaType.APPLICATION_JSON).body(
			studentService.getContractsByStudentId(studentId, Semester.builder().season(Semester.Season.valueOf(season))
			                                                          .year(Integer.parseInt(year)).build()));
	}
	...
	public ResponseEntity<ContractDTO> signContract(@RequestBody LoginDTO loginDTO, @PathVariable Long contractId){
		...
		return ResponseEntity.accepted().contentType(MediaType.APPLICATION_JSON).body(
			this.studentService.signContract(contractId, studentId));
	}
